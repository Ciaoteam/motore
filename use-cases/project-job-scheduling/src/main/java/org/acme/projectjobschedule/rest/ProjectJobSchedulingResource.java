...
import org.acme.projectjobschedule.rest.exception.ErrorInfo;
import org.acme.projectjobschedule.rest.exception.ScheduleSolverException;
...
public class ProjectJobSchedulingResource {
    ...
    public ProjectJobSchedulingResource() {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @Path("{jobId}")
    public ProjectJobSchedule getSchedule(
            @Parameter(description = "The job ID returned by the POST method.") @PathParam("jobId") String jobId) {
        ProjectJobSchedule schedule = getScheduleAndCheckForExceptions(jobId);
        SolverStatus solverStatus = solverManager.getSolverStatus(jobId);
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @Path("{jobId}/status")
    public ProjectJobSchedule getStatus(
            @Parameter(description = "The job ID returned by the POST method.") @PathParam("jobId") String jobId) {
        ProjectJobSchedule schedule = getScheduleAndCheckForExceptions(jobId);
        SolverStatus solverStatus = solverManager.getSolverStatus(jobId);
        return new ProjectJobSchedule(schedule.getScore(), solverStatus);
    }
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @Path("{jobId}")
    public ProjectJobSchedule terminateSolving(
            @Parameter(description = "The job ID returned by the POST method.") @PathParam("jobId") String jobId) {
        solverManager.terminateEarly(jobId);
        return getSchedule(jobId);
    }

    private ProjectJobSchedule getScheduleAndCheckForExceptions(String jobId) {
        Job job = jobIdToJob.get(jobId);
        if (job == null) {
            throw new ScheduleSolverException(jobId, Response.Status.NOT_FOUND, "No schedule found.");
        }
    ...
    private record Job(ProjectJobSchedule schedule, LocalDateTime createdAt, Throwable exception) {

        static Job ofSchedule(ProjectJobSchedule schedule) {
